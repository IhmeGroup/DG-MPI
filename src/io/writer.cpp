...
Writer::Writer(Mesh& mesh, MemoryNetwork& network, host_view_type_3D h_Uc,
    ...
    bool parallel = true;
    HDF5File file(name, H5F_ACC_TRUNC, parallel);
    ...
    file.create_and_write_dataset_parallel("Node Coordinates", dimensions, mesh.h_node_coords.data())

    /* -- Local to global node IDs -- */
    // Dimensions of the dataset: (num_nodes)
    dimensions.resize(1);
    dimensions[0] = mesh.num_nodes_part;
    file.create_and_write_dataset_parallel("Local to Global Node ID", dimensions, mesh.h_local_to_global_node_IDs.data());
    ...
    file.create_and_write_dataset_parallel("Element Global Node IDs", dimensions, mesh.h_elem_to_node_IDs.data());

    /* -- Local to global element IDs -- */
    // Dimensions of the dataset: (num_elems)
    dimensions.resize(1);
    dimensions[0] = mesh.num_elems_part;
    file.create_and_write_dataset_parallel("Local to Global Element IDs", dimensions, mesh.h_local_to_global_elem_IDs.data());
    ...
    dimensions[1] = nb;
    dimensions[2] = ns;
    file.create_and_write_dataset_parallel("Solution Coefficients", dimensions, h_Uc.data());
}
